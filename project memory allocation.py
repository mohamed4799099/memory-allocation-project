...
def buclikk():
    ...
    yy=[]
    yy=b.split(",")
    global starting_hole_address
    starting_hole_address=yy.copy()
    starting_hole_address=[int(i) for i in starting_hole_address]
...
def buclikkk():
    ...
    global size_hole
    size_hole=y.copy()
    size_hole=[int(i) for i in size_hole]
...
def best_fit_call():
    global v,Mem, Start, Size
    v=0
    Mem, Start, Size, token = bestfit(total_size, starting_hole_address, size_hole, name_process, name_segment, size_segment,Mem,Start,Size)
